...
However, interoperable management of graph data is challenging because the property graph model varies depending on each implementation.
...
For the purpose of exchange format, we allow wide range of property graphs such as graphs with arbitrary number of properties and labels. It also can be a mixed graph, which includes both directed and undirected edges. The property graph model has the following characteristics:
