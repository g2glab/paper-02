...
However, interoperable data management on these implementations is challenging due to the different data models being used.
...
Here, we redefined the property graph model to cover various database implementations and propose the serialization for property graphs. 
This model is independent from specific graph database implementations and provides an interoperable way to exchange property graph data. The proposed serialization is not only general and but also intuitive, thus it is useful for generating and maintaining property graph data. To show the practical usage of this general model and its serialization, we implemented converters from formats based on our model into existing formats, which can then be to load into various graph databases. This work provides an interoperable platform for generating, sharing and utilizing property graph data.
...
%there are no standardized data models for property graphs.
property graphs lack the standardized model.
Here, we surveyed the general requirements for representing property graph data and designed two common property graph formats as flat text and JSON. These formats can be converted into specific formats for each of the databases mentioned above. Common formats independent of certain database implementations will increase the interoperability of graph databases, and will make it easier for users to import accumulated graph data.

\section{Model Definition}
Here we define the property graph model independent of specific graph database implementations.
For the purpose of data exchange, we aim to cover a wide range of property graphs such as graphs with an arbitrary number of properties and labels, and mixed graphs with both directed and undirected edges. The property graph model has the following characteristics:

\begin{itemize}
    \item A property graph contains nodes and edges.
    \item Nodes and edges can have properties (key-value pairs).
    \item Each property can have multiple values.
    \item Nodes and edges can have zero or more labels.
...
\begin{enumerate}
    ...
    \item $l_v : N \to 2^S$ is a function mapping each node to its multiple labels.
...
According to the definition of the property graph model above, we propose data formats both in flat text and in JSON. While the flat text format (PG) is suited for human readability, the JSON format (JSON-PG) is best used for server-client communication.

The syntax of the flat text PG format is described below and an example is given in Figure~\ref{fig:example-pg}.
...
Further, Figure~\ref{fig:example-json} shows an example of the JSON-PG format. This format basically follows the rules of the general JSON format and the definition of the property graph above. However, labels and property values are described as arrays, while they are substantially lists of elements with no specific order.  
...
We have implemented a command-line tool to convert data between PG and JSON-PG, as well as to transform these formats into data loading formats for well-known graph databases such as Neo4j, Oracle Labs PGX, and Amazon Neptune. The actual use cases of this tool show that the proposed data format has the capability to describe general property graph data begin used in existing graph databases. (https://github.com/g2glab/pg)

\section{Conclusion}
In this work, we redefined the property graph model independent of specific graph database implementations and proposed practical data formats. Furthermore, we implemented tools to convert our formats into existing formats which will increase the interoperability of graph databases and make it easier for users to import accumulated graph data.
