...
\section{Definition}
Here we define the property graph model that the serialization format handles.
Since we aim at serialization of general property graphs, the model should be inclusive, that is, it should support many characteristics considered to be attributes of property graphs.
...
$PG = \langle V, E_u, E_d, S, P, s_e, t_e, e_e, l_v, l_e, p_v, p_e\rangle$, where:
\begin{enumerate}
    ...
    \item $s_e : E_d \to V$ is a function which yields the source of each directed edge,
    \item $t_e : E_d \to V$ is a function which yields the target of each directed edge,
    \item $e_e: E_u \to \langle V \times V \rangle$ is a function which yields the endpoints of each undirected edges,
    \item $l_v : V \to 2^S$ is a function mapping each nodes to their multiple labels,
    \item $l_e : (E_d \cup E_u) \to 2^S$ is a function mapping each edge to their multiple labels,
    \item $p_v : V \to 2^P$ is a function used to assign nodes to their multiple properties.
    \item $p_e : (E_d \cup E_u) \to 2^P$ is a function used to assign edges to their multiple properties.
\end{enumerate}
\end{defi}

\begin{table}[htb]
  \begin{tabular}{lcrr}
    Neo4j & PGX & igraph & networkX \\
  \end{tabular}
\end{table}
